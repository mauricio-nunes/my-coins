...
@php
    ...
    $editRoute = $transaction['card_installment']
        ...
        : ($transaction['statement_payment']
            ...
            : ($isTransfer ? route('transfers.edit', $transaction['id']) : route('transactions.edit', $transaction['id'])));
@endphp
...
@section('page_actions')<a href="{{ $editRoute }}" class="btn btn-primary"><i class="bi {{ $managedByCard ? 'bi-credit-card' : 'bi-pencil' }} me-1"></i> {{ $managedByCard ? 'Ver no cartão' : 'Editar' }}</a>@endsection
