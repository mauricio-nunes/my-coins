...
<div class="card border-0 shadow-sm"><div class="card-header bg-transparent border-0 d-flex justify-content-between align-items-center"><h2 class="h5 mb-0">Movimentações</h2><span class="badge text-bg-light">{{ $transactions->total() }} resultados</span></div>
    ...
    @forelse($transactions as $transaction)
        ...
        @endphp
        <tr><td><a href="{{ route('transactions.show', $transaction['id']) }}" class="fw-semibold text-body text-decoration-none">{{ $description }}</a><div class="small text-body-secondary">@if($isTransfer)<i class="bi bi-arrow-left-right me-1"></i>{{ $source['name'] }} → {{ $destination['name'] }}@else<i class="bi {{ $category['icon'] }} me-1"></i>{{ $category['name'] }}@endif</div>@if($transaction['tag_ids'] ?? [])<div class="d-flex flex-wrap gap-1 mt-2">@foreach($transaction['tag_ids'] as $tagId) @if($tag = $tags->firstWhere('id', $tagId))<span class="badge text-bg-light border fw-normal">#{{ $tag['name'] }}</span>@endif @endforeach</div>@endif</td><td>{{ $isTransfer ? $source['name'].' → '.$destination['name'] : $account['name'] }}</td><td>{{ \Carbon\Carbon::parse($transaction['date'])->format('d/m/Y') }}</td><td><span class="badge rounded-pill {{ $typeClass }}">{{ $typeLabel }}</span></td><td class="text-end fw-semibold {{ $transaction['type'] === 'income' ? 'text-success' : ($transaction['type'] === 'expense' ? 'text-danger' : 'text-body') }}">@if($isTransfer)<x-money :value="$transaction['amount']" />@else<x-money :value="($transaction['type'] === 'income' ? 1 : -1) * $transaction['amount']" :signed="true" />@endif</td><td class="text-end"><a href="{{ $isTransfer ? route('transfers.edit', $transaction['id']) : route('transactions.edit', $transaction['id']) }}" class="btn btn-sm btn-outline-secondary" aria-label="Editar {{ $description }}"><i class="bi bi-pencil"></i></a></td></tr>
