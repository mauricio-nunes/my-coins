...
<div class="card border-0 shadow-sm mb-4"><div class="card-body">
    <form method="get" class="row g-3 align-items-end">
        ...
        <div class="col-sm-6 col-lg-2"><label for="account_id" class="form-label">Conta</label><select id="account_id" name="account_id" class="form-select"><option value="">Todas</option>@foreach($accounts->where('type', '!=', 'credit_card') as $account)<option value="{{ $account['id'] }}" @selected(($filters['account_id'] ?? '') == $account['id'])>{{ $account['name'] }}</option>@endforeach</select></div>
        ...
        <div class="col-sm-6 col-lg-2"><label for="reconciled" class="form-label">Conciliada</label><select id="reconciled" name="reconciled" class="form-select"><option value="">Todas</option><option value="yes" @selected(($filters['reconciled'] ?? '') === 'yes')>Sim</option><option value="no" @selected(($filters['reconciled'] ?? '') === 'no')>Não</option></select></div>
