...
<div class="row g-4 mb-4"><div class="col-xl-7"><div class="card border-0 shadow-sm h-100"><div class="card-header border-0 bg-transparent"><h2 class="h5 mb-0">Evolução mensal</h2></div><div class="card-body"><div data-apexchart data-apexchart-config="{{ json_encode($trendChart) }}"></div></div></div></div>
<div class="col-xl-5"><div class="card border-0 shadow-sm h-100"><div class="card-header border-0 bg-transparent"><h2 class="h5 mb-0">Despesas por categoria</h2></div><div class="card-body">@if($report['byCategory']->isNotEmpty())<div data-apexchart data-apexchart-config="{{ json_encode($categoryChart) }}"></div>@else<div class="text-center py-5 text-body-secondary"><i class="bi bi-pie-chart display-5"></i><p class="mt-3">Sem despesas no período.</p></div>@endif</div></div></div></div>

<div class="card border-0 shadow-sm"><div class="card-header bg-transparent border-0"><h2 class="h5 mb-0">Detalhamento por categoria</h2></div><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Categoria</th><th class="text-end">Total</th><th class="text-end">Participação</th></tr></thead><tbody>@forelse($report['byCategory'] as $item)<tr><td>{{ $item['name'] }}</td><td class="text-end"><x-money :value="$item['amount']" /></td><td class="text-end">{{ $report['expenses'] ? number_format($item['amount']/$report['expenses']*100,1,',','.') : 0 }}%</td></tr>@empty<tr><td colspan="3" class="text-center py-4">Nenhum dado para exibir.</td></tr>@endforelse</tbody></table></div></div>
@endsection
