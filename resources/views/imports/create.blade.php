...
<div class="row justify-content-center"><div class="col-xl-9">
    ...
    <div class="card border-0 shadow-sm"><div class="card-body p-4 p-lg-5">
        ...
        <form method="post" action="{{ route('imports.preview') }}" enctype="multipart/form-data">
            @csrf
            <div class="row g-4">
                <div class="col-12"><label for="bank_format" class="form-label">Banco do arquivo</label><select id="bank_format" name="bank_format" class="form-select @error('bank_format') is-invalid @enderror" required><option value="">Selecione o banco que gerou o OFX</option>@foreach($bankFormats as $value => $label)<option value="{{ $value }}" @selected(old('bank_format') === $value)>{{ $label }}</option>@endforeach</select><div class="form-text">O formato selecionado será validado pelo código do banco presente no arquivo.</div><x-field-error name="bank_format" /></div>
