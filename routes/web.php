...
use App\Http\Controllers\Finance\CardPurchaseController;
use App\Http\Controllers\Finance\CardStatementPaymentController;
...
Route::middleware(['auth', 'password.changed'])->group(function (): void {
    ...
    Route::get('/credit-cards/{credit_card}/purchases/create', [CardPurchaseController::class, 'create'])->name('credit-cards.purchases.create');
    Route::post('/credit-cards/{credit_card}/purchases', [CardPurchaseController::class, 'store'])->name('credit-cards.purchases.store');
    Route::resource('card-purchases', CardPurchaseController::class)->only(['show', 'edit', 'update', 'destroy']);
    Route::post('/card-statements/{card_statement}/payments', [CardStatementPaymentController::class, 'store'])->name('card-statements.payments.store');
    Route::delete('/card-statement-payments/{card_statement_payment}', [CardStatementPaymentController::class, 'destroy'])->name('card-statement-payments.destroy');
